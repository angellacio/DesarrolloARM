...
namespace Sat.CreditosFiscales.Procesamiento.PagosV.NegocioPagos
{
    ...
    public class ProcesamientoArchivos
    {
        ...
        private long ProcActual;
        private long IdArchiProcesa;
        ...
        {
            ProcActual = idProceso;

            var archivosProcesar = new Dictionary<string, object>();
            var archivoGuardar = new Dictionary<string, object>();
            SIAT siat = new SIAT();

            //YK: LISTA DE ARCHIVOS Q
            ...
            //AJGG-8241 Agregar for each para guardado(registro de archivos)
            foreach (var archivoG in listaArchivos)
            {
                archivoGuardar = ObtenerArchivosProcesar(rutaRepositorio, archivoG);
                RegistrarArchivos(idProceso, archivoGuardar);
            }


            // Comienza el nuevo proceso de archivos 
            bool regresa = true;
            ...
            while (regresa)
            {
                //Nuevo metodo de obtener el archivo a procesar 8241
                TblControlPagosHead ArchivoQ = new TblControlPagosHead();
                ArchivoQ = SIATDALPagos.ObtieneNombreArchivoaProcesar(idProceso, NombreEquipo);

                IdArchiProcesa = ArchivoQ.idArchivo;
                // Obtiene el nombre del archivo a procesar
                if (ArchivoQ.NombreArchivo != null && !ArchivoQ.NombreArchivo.Equals(""))
                {

                    archivosProcesar = ObtenerArchivosProcesar(rutaRepositorio, ArchivoQ.NombreArchivo);
                    if (AplicarReglasDeValidacion(rutaRepositorio, archivosProcesar, idProceso))
                    {
                        if (GeneracionLineasDePagoParaSIR(idProceso, archivosProcesar, esReproceso)
                            && siat.GeneracionLineasDePago(idProceso, archivosProcesar, esReproceso, IdArchiProcesa))
                        {
            ...
            //Termina el procesado de archivo
        }
        ...
        private bool GeneracionLineasDePagoParaSIR(long idProceso, Dictionary<string, object> archivosProcesar, bool esReproceso)
        {
            ...
            bool ProcesarSIR = Boolean.Parse(ConfigurationManager.AppSettings["ProcesaSIR"]);


            try
            {

                // AJGG Validar prametro si pricesa SIR
                if (ProcesarSIR)
                {
                    if (archivosProcesar != null && archivosProcesar.Count > 0)
                    {
                        ...
                        ManejadorArchivos.WriteMessage("Procesando SIR archivo: " + detalle.Key);
                        listaArchivosTemp = ProcesaArchivo(cifraControl, detalle, idProceso); //FGR pensar en los diferentes layouts por tipo de crédito.                    
                ...
                // TErmina validación parametro procesa SIR
        ...
        private List<InfoArchivoSIR> ProcesaArchivo(KeyValuePair<string, object> cifraControl, KeyValuePair<string, object> detalle, long idProceso)
        {
            ...
            try
            {
                ...
                {
                    lineasDeCaptura = ValidarLineasDeCaptura(lineasDeCaptura, detalle, false, idProceso);
        ...
        public List<TblLineaCaptura> ValidarLineasDeCaptura(List<TblLineaCaptura> lineasDeCaptura, KeyValuePair<string, object> detalle, bool esCapturaMasiva, long IdProceso)
        {
            try
            {
                ...
                foreach (TblLineaCaptura tblLineaCaptura in lineasDeCaptura)
                {
                    ...
                    else
                    {
                        ...
                        SIATDALPagos.ActualizaErrorProcesoLC(ProcActual, IdArchiProcesa, tblLineaCaptura.LineaCaptura, (int)EnumErroresProceso.MontoInvalido);
        ...
        private bool AplicarReglasDeValidacion(string rutaRepositorio, Dictionary<string, object> archivosProcesar, long idProceso)
        {
            ...
            if (ValidarArchivoQBancoFecha(archivoQ, ref archivosError))
            {
                ...
                else
                {
                    ...
                    //                    SIATDALPagos.ActualizaEstadoProcesoLinea()
                ...
                //    SIATDALPagos.ActualizaErrorProceso(idProceso, IdArchiProcesa, (int)EnumErroresProceso.FechasOperacion);
        ...
        private bool ValidarArchivoDetalles(KeyValuePair<string, object> archivoQ, KeyValuePair<string, object> archivoE, ref Dictionary<string, object> archivosError)
        {
            try
            {
                ...
                if (contenidoCifraControl[2].Trim() != numLineasCaptura.ToString())
                {
                    ...
                    SIATDALPagos.ActualizaErrorProceso(ProcActual, IdArchiProcesa, (int)EnumErroresProceso.NumeroRegistros);
                ...
                if (contenidoCifraControl[3].Trim() != sumaOperaciones.ToString())
                {
                    ...
                    SIATDALPagos.ActualizaErrorProceso(ProcActual, IdArchiProcesa, (int)EnumErroresProceso.SumaImportes);
                ...
                if (fechaEncabezado.ToString("yyyyMMdd") != fechaArchivo.ToString("yyyyMMdd"))
                {
                    ...
                    SIATDALPagos.ActualizaErrorProceso(ProcActual, IdArchiProcesa, (int)EnumErroresProceso.FechaInvalida);
                ...
                if (claveBanco != encabezado[2].Trim())
                {
                    ...
                    SIATDALPagos.ActualizaErrorProceso(ProcActual, IdArchiProcesa, (int)EnumErroresProceso.BancoNoCoincide);
                ...
                if (Boolean.Parse(ConfigurationManager.AppSettings["EvaluarVersiones"]))
                {
                                ...
                                lineaTexto => lineaTexto.Split('|')))
                    {
                        if (!versionesValidas.Contains(arrDetalles[8]))
                        {
                            ...
                            SIATDALPagos.ActualizaErrorProceso(ProcActual, IdArchiProcesa, (int)EnumErroresProceso.VersionInvalida);
                            break;
                        }
                    }
                }
            ...
            try
            {
                ...
                foreach (string cadena in contenido)
                {
                    ...
                    if (contenido.Length <= 1)
                    {
                        ...
                        //           SIATDALPagos.ActualizaEstadoProceso(ProcActual, archivoQ.Key, 2);  // CatErrorProceso
                    ...
                    if (!ExisteBanco(linea[0]))
                    {
                        ...
                        SIATDALPagos.ActualizaErrorProceso(ProcActual, IdArchiProcesa, (int)EnumErroresProceso.NoExisteBanco);
        ...
        private void RegistrarArchivos(long IdProceso, Dictionary<string, object> archivoaGuardar)
        {
            var archivoQ = archivoaGuardar.FirstOrDefault(archivo => archivo.Key.StartsWith("Q"));
            var archivoE = archivoaGuardar.FirstOrDefault(archivo => archivo.Key.StartsWith("E"));
            var archivosError = new Dictionary<string, object>();

            //  RegistraArchivoQ(archivoQ);
            ...
            try
            {
                ...
                foreach (string cadena in contenido)
                {
                    ...
                    TblControlPagosHead rArchivoQ = new TblControlPagosHead();
                    var fechaControlText = linea[1];
                    ...
                    var impo = long.Parse(linea[3].ToString());

                    rArchivoQ.IdProceso = IdProceso;
                    rArchivoQ.NombreArchivo = archivoQ.Key.Replace("Q", "");
                    rArchivoQ.IdBanco = Convert.ToInt32(linea[0].ToString());
                    rArchivoQ.FechaPago = fechaControl.ToString("yyyyMMdd");
                    rArchivoQ.NumRegistros = Convert.ToInt32(linea[2].ToString());
                    rArchivoQ.Importe = impo;
                    rArchivoQ.FechaRegistro = DateTime.Now;
                    //      rArchivoQ.FechaProceso = fechaControl;
                    rArchivoQ.Estado = 1;


                    idArchi = SIATDALPagos.RegistraArchivoQ(IdProceso, rArchivoQ);

                    //AJGG 8241 Guardar en BD y obtener el idARchvios


                    //AJGG GUARDAR EL DETALLE


                    var contenidoE = (string[])archivoE.Value;
                    int nl = 1;
                    if (contenidoE != null)
                    {
                        foreach (string cadenaE in contenidoE)
                        {

                            if (cadenaE.Trim().ToUpper() == Constantes.EOF) continue;

                            var lineaE = cadenaE.Split('|');

                            TblControlPagosDet rArchivoE = new TblControlPagosDet();

                            if (lineaE[0].Equals("1")) continue;


                            rArchivoE.IdArchivo = idArchi;
                            rArchivoE.NumLinea = nl;
                            rArchivoE.Consecutivo = lineaE[1];
                            rArchivoE.LineaCaptura = lineaE[2];
                            rArchivoE.FechaPago = lineaE[3];
                            rArchivoE.HoraPago = lineaE[4];
                            rArchivoE.Importe = lineaE[5];
                            rArchivoE.NumOperacion = lineaE[6];
                            rArchivoE.MedioRecepcion = Convert.ToInt32(lineaE[7]);
                            rArchivoE.Version = lineaE[8];
                            rArchivoE.IdEstado = 1;
                            rArchivoE.TipoPago = 1;

                            // COdigo para guardar

                            SIATDALPagos.RegistraArchivoE(rArchivoE, false);
                            nl++;
                    ...
                    {
                        SIATDALPagos.ActualizaErrorProceso(IdProceso, idArchi, (int)EnumErroresProceso.SinArchivoE);
                    }
            ...
            {
                ManejadorArchivos.WriteMessage("Error al registrar archivo: " + archivoQ.Key + "|" + ex.Message);
            }
