<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:var="http://schemas.microsoft.com/BizTalk/2003/var" exclude-result-prefixes="msxsl var userCSharp" version="1.0" xmlns:userCSharp="http://schemas.microsoft.com/BizTalk/2003/userCSharp">
  ...
  <xsl:template match="/CreditosFiscales">
    ...
    <xsl:variable name="var:v4" select="count(/CreditosFiscales/Conceptos/Concepto)" />
    ...
    <SolicitudGeneracionLC>
      <DatosGenerales>
        ...
        <ClaveGenerica>
          <xsl:value-of select="DatosGenerales/SolicitudARectificar/text()" />
        </ClaveGenerica>          
        <NumeroConceptos>
      ...
      <Conceptos>       
          <xsl:for-each select="Conceptos/Concepto">
            ...
            <Concepto>
              <xsl:choose>
                <xsl:when test="EsCorrecto/text() = 'true'">
                  <NumeroSecuencia>2</NumeroSecuencia>
                </xsl:when>
                <xsl:otherwise>
                  <NumeroSecuencia>1</NumeroSecuencia>
                </xsl:otherwise>
              </xsl:choose>             
              <Clave>
              ...
              <DatosIcep>
                <xsl:choose>
                  <xsl:when test="EsCorrecto/text() = 'true'">
                    <IcepCorrecto>1</IcepCorrecto>
                  </xsl:when>
                  <xsl:otherwise>                                          
                    <IcepCorrecto>0</IcepCorrecto>                    
                  </xsl:otherwise>
                </xsl:choose>
                <xsl:choose>
