...
namespace SAT.CreditosFiscales.Motor.AccesoDatos.Busquedas
{
	...
	public class BusquedaLineaCaptura
	{
		...
		public static List<RespuestaListaFormatos> ConsultaFormatosXFiltros
																			(
																				int ADR
																				, string RFC
																				, string Folio
																				...
																				, int rango_de_emision_FechaPago
																				, DateTime fecha_ini
																				, DateTime fecha_fin
																			)
		{
						...
						new RespuestaListaFormatos
							{
								...
								, NumResolucion = (Convert.ToString(Formato["ValorAgrupador"]).Split('|').Length > 0) ? Convert.ToString(Formato["ValorAgrupador"]).Split('|')[0] : string.Empty //regla de split
		...
		public static List<string> ConsultaProcesamientPorDatosGenerales
																		(
																			string RFC
																			, int ALR
																			, int TipoDocumento
		...
		public static List<DatosProcesamiento.ConceptoOriginalDB> ConsultaConceptosPorAgrupador
																						(
																							int idAgrupador
		...
		public static List<DatosProcesamiento.ConceptoOriginalHijo> ConsultaConceptosHijoPorAgrupador
																						(
																							int idAgrupador
																							, Guid idProcesamiento
		...
		public static List<DatosProcesamiento.DescuentosOriginal> ConsultaDescuentosPorConcepto
																						(
																							int idAgrupador
																							, Guid idProcesamiento
																							, int idConceptoOriginal
		...
		public static List<DatosProcesamiento.DescuentoOriginalHijo> ConsultaDescuentosPorConceptoHijo
																	(
																		int idAgrupador
																		, Guid idProcesamiento
																		, int idConceptoOriginal
		...
		public static RespuestaLC ConsultaLineaCapturaPorProcesamiento(Guid idProcesamiento)
		{
			...
			byte idAplicacion = 0;
			var vLineaCaptura = new RespuestaLC();

			db.AddInParameter(cmd, "@IdProcesamiento", System.Data.DbType.Guid, idProcesamiento);

			using (var rdReglas = db.ExecuteReader(cmd))
			{
				vLineaCaptura.LineasCaptura = new RespuestaLCDatosLinea[1];
				var vDatosLineaCaptura = new RespuestaLCDatosLinea();

				while (rdReglas.Read())
				{
					vDatosLineaCaptura.Folio = rdReglas["FolioDyP"].ToString();
					vDatosLineaCaptura.LineaCaptura = rdReglas["LineaCaptura"].ToString();
					idEstatusPago = Byte.Parse(rdReglas["IdEstatus"].ToString());
					idAplicacion = Byte.Parse(rdReglas["IdAplicacion"].ToString());
				}

				vLineaCaptura.LineasCaptura[0] = vDatosLineaCaptura;

				if (idEstatusPago == 2 && idAplicacion == 5)
				{
					vLineaCaptura.ListaErrores = new String[1];
					vLineaCaptura.ListaErrores[0] = "La línea de captura fue previamente pagada";
				}
			}

			return vLineaCaptura;
		}
		...
		public static RespuestaSolicitudOriginal ConsultaXMLOriginal(string Folio)
		{
			...
			using (var vErXmlvOriginal = db.ExecuteReader(cmd))
			{

				while (vErXmlvOriginal.Read())
				{
					respuesta.XmlOriginal = vErXmlvOriginal["Mensaje"].ToString();
					respuesta.LineaCaptura = vErXmlvOriginal["LineaCaptura"].ToString();
					respuesta.FechaPago = vErXmlvOriginal["FechaPago"].ToString();
					respuesta.FechaEmision = vErXmlvOriginal["FechaEmision"].ToString();
				}
