...
namespace SAT.CreditosFiscales.Motor.Negocio.Procesamiento
{
	...
	public class LogicaAlmacena
	{
		...
		#region Métodos Públicos
			/// <summary>
			/// Almacena los datos del objeto canónico en base de datos
			/// </summary>
			/// <param name="oCreditosFiscales">objeto canónico con los datos a insertar</param>
			/// <returns></returns>
			public Respuesta<string> AlmacenaDatosBD(DatosProcesamiento.CreditosFiscales oCreditosFiscales)
			{
				var resultadoInserta = new Respuesta<string>();

				try
				{
					resultadoInserta.EsExitoso = DalDatosGenerales.InsertaDatosGenerales(
																							IdProcesamiento, Aplicacion, oCreditosFiscales.DatosGenerales.RFC
					...
					if (resultadoInserta.EsExitoso)
					{
							...
							foreach (DatosProcesamiento.CreditosFiscalesAgrupador agrupador in oCreditosFiscales.Agrupadores)
							{
								...
								if (DalAgrupador.InsertaAgrupador(IdProcesamiento, idAgrupador, ValorAgrupador, TotalAgrupador, agrupador.SaldoInformativo))
								{
									if (agrupador.ConceptosOriginal.Count() > 0)
									{
										...
										{
											string IdConceptoPadre = DalConceptos.InsertaConceptoOriginal(
																											IdProcesamiento
																											, idAgrupador
																											, conceptoPadre.Clave
																											, conceptoPadre.Ejercicio
																											, conceptoPadre.PeriodicidadId.ToString()
																											, conceptoPadre.PeriodoId.ToString()
																											, conceptoPadre.FechaCausacion
																											, conceptoPadre.CreditoSir
																											, conceptoPadre.ImporteHistorico
																											, conceptoPadre.ImporteParteActualizada
																											, conceptoPadre.ImportePagar
																											, conceptoPadre.Liquidar
																											, conceptoPadre.MotivoId
																											, conceptoPadre.EsCorrecto
																										);
											...
											if (IdConceptoPadre != string.Empty)
											{
												if (conceptoPadre.Descuentos != null)
												{
													...
													foreach (DatosProcesamiento.CreditosFiscalesAgrupadorConceptosDescuento descuentoOriginal in conceptoPadre.Descuentos)
														DalDescuentos.InsertaDescuentoConceptoOriginal
																			(
																				idAgrupador
																				, IdProcesamiento
																				, Convert.ToInt32(IdConceptoPadre)
																				, descuentoOriginal.IdDescuento
																				, descuentoOriginal.ImporteDescuento
																			);
												}
												...
												if (conceptoPadre.Hijos != null)
												{
													...
													{
														string IdConceptoHijo = DalConceptos.InsertaConceptoHijo
																				(
																					IdProcesamiento
																					, idAgrupador
																					, Convert.ToInt32(IdConceptoPadre)
																					...
																					, conceptoHijo.Ejercicio
																					, conceptoHijo.FechaCausacion
																					, conceptoHijo.PeriodicidadId
																					, conceptoHijo.PeriodoId
																					, conceptoHijo.Liquidar
																					, conceptoHijo.MotivoId
																					, conceptoHijo.TipoTransaccion
																					, conceptoHijo.CreditoSir
																				);
														...
														if (IdConceptoHijo != string.Empty && conceptoHijo.Descuentos != null)
														{
															foreach (DatosProcesamiento.CreditosFiscalesAgrupadorConceptosHijoDescuento descuentoHijo in conceptoHijo.Descuentos)
																DalDescuentos.InsertaDescuentoConceptoOriginalHijo
																		(
																			idAgrupador
																			, IdProcesamiento
																			, Convert.ToInt32(IdConceptoPadre)
																			, Convert.ToInt32(IdConceptoHijo)
																			, descuentoHijo.IdDescuento
																			, descuentoHijo.ImporteDescuento
																		);
														}
													}
												}
						...
						if (oCreditosFiscales.Conceptos != null)
						{
							if (oCreditosFiscales.Conceptos.Count() > 0)
							{
								...
								{
									string IdConcepto = DalConceptos.InsertaConceptoDyP(IdProcesamiento, concepto.Clave, concepto.Ejercicio, concepto.ClaveOrigen, concepto.TipoPeriodicidad, concepto.ClavePeriodo, concepto.NumeroCredito, concepto.FechaCausacion);
...
/*
						if (oCreditosFiscales.Conceptos != null)
						{
							if (oCreditosFiscales.Conceptos.Count() > 0)
							{
								...
								foreach (DatosProcesamiento.CreditosFiscalesConcepto concepto in oCreditosFiscales.Conceptos)
								{
									string IdConcepto = DalConceptos.InsertaConceptoDyP
														(
															IdProcesamiento
															, concepto.Clave
															, concepto.Ejercicio, concepto.ClaveOrigen
															, concepto.TipoPeriodicidad
															, concepto.ClavePeriodo
															, concepto.NumeroCredito
