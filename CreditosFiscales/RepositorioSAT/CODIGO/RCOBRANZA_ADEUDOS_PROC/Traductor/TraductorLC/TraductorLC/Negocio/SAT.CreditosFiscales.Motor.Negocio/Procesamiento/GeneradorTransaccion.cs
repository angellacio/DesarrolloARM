...
namespace SAT.CreditosFiscales.Motor.Negocio.Procesamiento
{
	...
	public class GeneradorTransaccion
	{
		...
		public CreditosFiscalesConceptoTransaccion GeneraTransaccion(int idAplicacion, int idTipodocumento, string valorObjeto, decimal importe, string objetoSerializado)
		{
			string idTransaccion = "-1";
			//var lstReglas = DalConceptosEquivalencia.ObtenerReglasEquivalencia(idAplicacion, EnumTipoObjeto.Transaccion, idTipodocumento, valorObjeto);            

			List<CatReglasEquivalencia> lstReglas = HerramientasReglasEquivalencia.ObtenerReglasEquivalencia(idAplicacion, EnumTipoObjeto.Transaccion, valorObjeto, idTipodocumento);
		...
		//	XDocument objetoFiltro = XDocument.Parse(objetoSerializado);
			...
			{
				DtdProcessing = DtdProcessing.Prohibit, // Prohibir DTDs para prevenir ataques XXE
				XmlResolver = null // Deshabilitar la resolución de entidades externas
			};

			XDocument objetoFiltro;
			using (var reader = XmlReader.Create(new StringReader(objetoSerializado), settings))
			{
				objetoFiltro = XDocument.Load(reader); // Cargar el XML de manera segura
			}
			...
			if (idTransaccion == "-1")
			{
				string mensaje = string.Format("No existe transacción definida en el catálogo de reglas para el valor objeto {0}", valorObjeto);
				LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
				throw new Exception(mensaje);
			}

			CatTransaccion transaccion = DalConceptosEquivalencia.ObtenerTransaccionPorId(idTransaccion, idAplicacion, idTipodocumento);
			if (transaccion == null)
			{
				string mensaje = string.Format("No existe transacción definida en el catálogo de transacciones para la transacción {0} y el valor objeto {1}", idTransaccion, valorObjeto);
				LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
				throw new Exception(mensaje);
			}
		...
		public CreditosFiscalesConceptoTransaccion GeneraTransaccion(int idAplicacion, int idTipodocumento, string valorObjeto, CreditosFiscalesAgrupadorConceptosDescuento descuento)
		{
			...
			var settingss = new XmlReaderSettings
			{
				DtdProcessing = DtdProcessing.Prohibit, // Prohibir DTDs para prevenir ataques XXE
				XmlResolver = null // Deshabilitar la resolución de entidades externas
			};
			...
			var doc = new XmlDocument();
			using (var reader = XmlReader.Create(new StringReader(MetodosComunes.SerlializaObjeto(descuento)), settingss))
			{
				doc.Load(reader); // Cargar el XML de manera segura
			}
			...
			//var lstReglas = RecuperarReglasDescuento(idAplicacion, idTipodocumento, EnumTipoObjeto.Transaccion, valorObjeto);
			List<CatReglasEquivalencia> lstReglas = HerramientasReglasEquivalencia.ObtenerReglasEquivalencia(idAplicacion, EnumTipoObjeto.Transaccion, valorObjeto, idTipodocumento);

			string idTransaccion = "-1";
			//XDocument objetoFiltro = XDocument.Parse(doc.InnerXml);
			...
			if (transaccion == null)
			{
				string mensaje = string.Format("No existe transacción definida en el catálogo de transacciones para el idtransacción {0} y valor objeto {1}", idTransaccion, valorObjeto);
				LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
				throw new Exception(mensaje);
			}
		...
		/// 
		/// </summary>
		/// <param name="catTransaccion"></param>
...
//				return;
			...
			try
			{
				ListaReglasDescuentos = DalConceptosEquivalencia.ObtenerReglasEquivalencia(idAplicacion, transaccion, idTipoDocumento, valorObjeto);
			}
		...
		public CreditosFiscalesConceptoTransaccion GeneraTransaccion(int idAplicacion, int idTipodocumento, string valorObjeto, CreditosFiscalesAgrupadorConceptosHijoDescuento descuento)
		{
			...
			var doc = new XmlDocument();
			using (var reader = XmlReader.Create(new StringReader(MetodosComunes.SerlializaObjeto(descuento)), settings))
			{
				doc.Load(reader); // Cargar el XML de manera segura
			}
			...
			if (transaccion == null)
			{
				string mensaje = string.Format("No existe transacción para el idtransacción {0} y el valor objeto {1}", idTransaccion, valorObjeto);
				LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
				throw new Exception(mensaje);
			}
			return CreaTransaccion(transaccion, importeDescuento);
		...
		{
			//XDocument objetoFiltro = XDocument.Parse(objetoSerializadoHijo);
			//List<CatReglasEquivalencia> listaReglas = RecuperarReglas(idAplicacion, idTipodocumento, EnumTipoObjeto.Transaccion, valorObjeto); // el valor objeto aqui debería ser el concepto Ley.
			...
			XDocument objetoFiltro;
			using (var reader = XmlReader.Create(new StringReader(objetoSerializadoHijo), settings))
			{
				objetoFiltro = XDocument.Load(reader); // Cargar el XML de manera segura
			}
			// END POL-8424
			...
			List<CatReglasEquivalencia> listaReglas = HerramientasReglasEquivalencia.ObtenerReglasEquivalencia(idAplicacion, EnumTipoObjeto.Transaccion, valorObjeto, idTipodocumento);
			string idTransaccion = "-1";
			idTransaccion = HerramientasReglasEquivalencia.AplicarReglasEquivalencia(objetoFiltro, listaReglas);
			if (idTransaccion == "-1")
			{
				var mensaje = string.Format("No existe transacción definida para el en el catálogo de reglas para el valor objeto {0}", valorObjeto);
				LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
				throw new Exception(mensaje);
			}
			...
			if (transaccion == null)
			{
				string mensaje = string.Format("No existe transacción definida en el catálogo de transacciones para el idtransacción {0} y el valor objeto {1}", idTransaccion, valorObjeto);
				LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
				throw new Exception(mensaje);
			}
		...
		private List<CatReglasEquivalencia> RecuperarReglas(int idAplicacion, int idTipodocumento, EnumTipoObjeto transaccion, string valorObjeto)
		{
			try
			{
				...
				return HerramientasReglasEquivalencia.ObtenerReglasEquivalencia(idAplicacion, transaccion, valorObjeto, idTipodocumento);
			}
		...
		public List<CreditosFiscalesConceptoTransaccion> GeneraTransaccion(int idAplicacion, sbyte idTipodocumento, string objetoSerializado)
		{
			...
			//XDocument objetoFiltro = XDocument.Parse(objetoSerializado);
			//List<CatReglasEquivalencia> listaReglas = RecuperarReglas(idAplicacion, idTipodocumento, EnumTipoObjeto.Transaccion, Constantes.TotalMontoCargo);


			var listaTransacciones = new List<CreditosFiscalesConceptoTransaccion>();

			// Sanitizar y validar el XML antes de procesarlo
			var settings = new XmlReaderSettings
			{
				DtdProcessing = DtdProcessing.Prohibit, // Prohibir DTDs para prevenir ataques XXE
				XmlResolver = null // Deshabilitar la resolución de entidades externas
			...
			foreach (var regla in listaReglas)
			{
				...
				if (nodoObjeto != null)
				{
					string idTransaccion = regla.ValorRetorno;
					long importe = Convert.ToInt64(nodoObjeto.Value);
					CatTransaccion catTransaccion = DalConceptosEquivalencia.ObtenerTransaccionPorId(idTransaccion, idAplicacion, idTipodocumento);
					if (catTransaccion == null)
					{
						string mensaje = string.Format("No existe transacción definida para este identificador de transacción {0}", idTransaccion);
						LogEventos.EscribirEntradaLog((int) EnumErrorMotor.ErrorCrearTransaccion, mensaje, EventLogEntryType.Error);
						throw new Exception(mensaje);
					}
					CreditosFiscalesConceptoTransaccion transaccion = CreaTransaccion(catTransaccion, Convert.ToInt64(importe));
