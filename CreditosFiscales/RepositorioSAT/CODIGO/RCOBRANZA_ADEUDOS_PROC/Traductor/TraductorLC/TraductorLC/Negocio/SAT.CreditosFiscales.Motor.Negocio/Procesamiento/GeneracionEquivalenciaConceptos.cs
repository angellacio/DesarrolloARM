...
*/
		...
		private static void ObtenerPeriodoEquivalencia(int idAplicacion, ref CreditosFiscalesConcepto concepto, CreditosFiscalesAgrupadorConceptos conceptoOriginal)
		{
			...
			if (esAplicarRegla)
			{
				...
				using (var reader = XmlReader.Create(new StringReader(MetodosComunes.SerlializaObjeto(conceptoOriginal)), settings))
				{
		...
		private static void ObtenerPeriodicidadEquivalencia(int idAplicacion, ref CreditosFiscalesConcepto concepto, CreditosFiscalesAgrupadorConceptos conceptoOriginal)
		{
			...
			if (esAplicarRegla)
			{
				...
				if (idPeriodicidad == "-1")
				{
					string mensaje = string.Format("No existe periodicidad dyp para la periodicidadId origen {0} en la aplicaciónId {1}", conceptoOriginal.PeriodicidadId, idAplicacion);
					LogEventos.EscribirEntradaLog((int)EnumErrorMotor.ErrorCrearConceptoDyP, mensaje, EventLogEntryType.Error);
					throw new Exception(mensaje);
				}
		...
		private static string GenerarPeriodoDyP(int idAplicacion, string objetoSerializable, int periodoId)
		{
			try
			{
				...
				using (var reader = XmlReader.Create(new StringReader(objetoSerializable), settings))
				{
					objetoFiltro = XDocument.Load(reader);
				}

				string idPeriodo = HerramientasReglasEquivalencia.AplicarReglasEquivalencia(objetoFiltro, lstReglasEquivalencia);
				return idPeriodo;
