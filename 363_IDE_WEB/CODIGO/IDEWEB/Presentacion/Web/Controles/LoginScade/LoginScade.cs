...
namespace Sat.Scade.Net.IDE.Presentacion.Web
{
    ...
    ToolboxData(Utility.ToolBoxData.LoginScade)]
    public class LoginScade : CompositeControl
    {
        ...
        [Category(Utility.Category.EDSCiec), ReadOnly(true)]
        public bool ActualizarMailCiec
        {
            get
            {
                if (this.ciec != null)
                {
                    return this.ciec.UpdateMail;
                }
                return false;
            }
        }
        ...
        [Bindable(true), Category(Utility.Category.EDSCiec), UrlProperty, Editor("System.Web.UI.Design.UrlEditor, System.Design, Version=2.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a", typeof(UITypeEditor))]
        public string UrlPaginaDestinoCiecMod
        {
            get
            {
                return (String)ViewState["Scade_UrlPaginaDestinoCiecMod"] ?? String.Empty;
            }
            set
            {
                ViewState["Scade_UrlPaginaDestinoCiecMod"] = value;
            }
        }
        ...
        protected override void CreateChildControls()
        {
            ...
            this.fiel = new LoginFiel();
            this.fiel.OnChangeLoginType += new CommandEventHandler(Scade_OnChangeLoginType);

            if (this.LoggingInFiel != null)
            {
                this.fiel.LoggingIn += new LoginCancelEventHandler(fiel_LoggingIn);
            }

            if (this.LoggedInFiel != null)
            {
                this.fiel.LoggedIn += new EventHandler(fiel_LoggedIn);
            }

            if (this.LoginErrorFiel != null)
            {
                this.fiel.LoginError += new EventHandler(fiel_LoginError);
            }

            if (this.AuthenticateFiel != null)
            {
                this.fiel.Authenticate += new AuthenticateEventHandler(fiel_Authenticate);
            }

            this.fiel.Width = this.AnchoApplet;
            this.fiel.Height = this.AltoApplet;
            this.fiel.JavaArchive = this.JavaArchive;
            this.fiel.JavaCode = this.JavaCode;
            this.fiel.UrlOriginal = this.UrlOriginal;
            this.fiel.AppPage = this.AppPage;
            this.fiel.JavaPlugIn = this.JavaPlugIn;

            this.fiel.MailVisible = this.VisualizarMailFiel;

            this.fiel.MailText = this.MailEtiquetaFiel;
            if (this.MailEtiquetaFiel.Length == 0)
            {
                this.fiel.MailText = null;
            }
            this.fiel.MailPageUrl = this.UrlPaginaCorreoFiel;

            this.fiel.CiecText = this.CiecEtiqueta;
            if (this.CiecEtiqueta.Length == 0)
            {
                this.fiel.CiecText = null;
            }

            this.fiel.CiecVisible = this.VisualizarCiec;

            this.fiel.LoginText = this.BotonLoginEtiquetaFiel;
            if (this.BotonLoginEtiquetaFiel.Length == 0)
            {
                this.fiel.LoginText = null;
            }

            this.fiel.FailureText = this.ErrorAutenticacionFiel;
            this.fiel.MembershipProvider = this.MembershipProviderFiel;
            this.fiel.DestinationPageUrl = this.UrlPaginaDestinoFiel;
            this.fiel.IsValidCertificate = this.IsValidCertificate;

            // R2. Agregamos la Url a la Fiel
            this.fiel.DestinationMailUpdateUrl = this.UrlPaginaDestinoUpdateMail;

            this.Controls.Add(this.fiel);
            
            this.ciec = new LoginCiec();
            this.ciec.OnChangeLoginType += new CommandEventHandler(Scade_OnChangeLoginType);
            ...
            this.ciec.ButtonText = this.BotonLoginEtiquetaCiec;
            this.ciec.FielTitle = this.FielTitulo;
            this.ciec.FielTextUrl = this.FielEtiqueta;
            ...
            if ((int)Parametros.MedioPresentacion == 2)
            {
                this.ciec.DestinationPageUrl = this.UrlPaginaDestinoCiecMod;
            }
            else
            {
            ...
            this.Controls.Add(this.ciec);
            
            this.fiel.Visible = this.Modo == LoginType.Fiel;
            this.ciec.Visible = this.Modo == LoginType.Ciec;
        }
